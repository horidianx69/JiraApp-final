...
//         : "bg-yellow-400 text-gray-800 hover:bg-yellow-500 shadow-yellow-200"
//     }`}
//             >
...
const TaskItem = ({ task, onRefresh, onLogout }) => {
  ...
  // Sync completed state with task prop
  useEffect(() => {
    setIsCompleted(!!task.completed);
  }, [task.completed]);
  ...
  return (
    <>
      <div className={`${TI_CLASSES.wrapper} ${borderColor}`}>
        {/* --- Left content (title, priority, description) --- */}
        <div className="flex-1 min-w-0">
          <div className="flex items-baseline gap-2 mb-1 flex-wrap">
            <h3
              className={`${TI_CLASSES.titleBase} ${
        ...
        {/* --- Right side (menu, date, created time) --- */}
        <div className={TI_CLASSES.rightContainer}>
          <div className="relative">
            <button
              onClick={() => setShowMenu(!showMenu)}
        ...
        <div className="flex justify-between items-center mt-3 gap-3">
          ...
          <button
            onClick={() => setShowEditModal(true)} // expand into modal
            className="px-4 py-2 rounded-full font-semibold bg-purple-500 text-white hover:bg-purple-600 transition-all duration-300 shadow-sm"
          >
            Expand
          </button>
        </div>
